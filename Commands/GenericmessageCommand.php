...
class GenericmessageCommand extends SystemCommand
{
    ...
    public function execute()
    {
        ...
        if (!file_exists($png_file_path)) {
            ...
            try {
                $api = new \CloudConvert\Api($cc_api_key);
                ...
                return $this->sendReply($chat_id, $message_id, 'Conversion failed, maybe because of a broken input file: ' . $e->getMessage());
        ...
        if ($this->stickerSetExists($sticker_set_name)) {
            ...
            if (!$this->checkOk($add_sticker_to_set_response)) {
            ...
            if (!$this->checkOk($create_sticker_set_response)) {
        ...
        if (!$this->checkOk($get_sticker_set_response)) {
    ...
    public function stickerSetExists($sticker_set_name) {
        try {
            ...
            if (!$get_sticker_set_response->isOk()) {
                $this->logMsg('Sticker set ' . $sticker_set_name . ' does not exist: ' . $get_sticker_set_response->getDescription());
                return false;
            } else {
                return true;
            }
    ...
    public function checkOk($response) {
        return $response->isOk();
    }
    
    public function logMsg($msg) {
        file_put_contents(__DIR__ . '/../logs/debug.log', $msg . PHP_EOL, FILE_APPEND | LOCK_EX);
    }
    
    public function errMsg($msg) {
        file_put_contents(__DIR__ . '/../logs/error.log', $msg . PHP_EOL, FILE_APPEND | LOCK_EX);
    }
